  ...
  if(uid == UID){    
    ...
    if(action == "setThreshold"){
      ...
      if(threshold_in > 0 && threshold_in <= 10000){
        ...
        sendResponse(action, false, "Threshold Wert nicht im Wertebereich.(1 - 10000PPM)");
      }
    }
    else{
...
void loop() {
  ...
  if(millis()-lastMillis > REFRESH_RATE_MS){
    lastMillis = millis();
    sendSensorData("AIRQUALITY",String(CO2));
  }

  //Raumluftstatus angeben
  if(CO2 > threshold){
    if(lueftenNotwendig == 0) sendEventData("checkAir","ok", "not_ok", "AIRQUALITY");
    lueftenNotwendig = 1;
  }
  else{
    if(lueftenNotwendig == 1) sendEventData("checkAir", "not_ok", "ok", "AIRQUALITY");
    lueftenNotwendig = 0;
  }
  drawRaumstatus();
}
