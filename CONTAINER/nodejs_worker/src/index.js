...
let client = mqtt.connect(`mqtt://${process.env.MQTT_HOST}`, options);
...
client.on('connect', () => {
  ...
  client.subscribe(process.env.MQTT_SENSORDATA_TOPIC, (e) => {
    if (e)
      console.log(
        `There was an error subscribing to ${process.env.MQTT_SENSORDATA_TOPIC}": ${e.message}`
      );
  });
});
